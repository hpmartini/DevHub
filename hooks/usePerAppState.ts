...
export function usePerAppState() {
  ...
  const [terminalStateMap, setTerminalStateMap] = useState<Map<string, PerAppTerminalState>>(
    new Map()
  );
  ...
  const getTerminalState = useCallback(
    (appId: string): PerAppTerminalState => {
  ...
  const updateTerminalState = useCallback(
    (appId: string, updates: Partial<PerAppTerminalState>) => {
   ...
   */
  const createAppStateHelpers = useCallback(
    (appId: string) => {
      ...
      return {
        ...
        setActiveTerminalTabId: (id: string | null) =>
          updateTerminalState(appId, { activeTabId: id }),
...
export type PerAppStateHelpers = ReturnType<
  ReturnType<typeof usePerAppState>['createAppStateHelpers']
>;
